...
class Product extends Model
{
    ...
    protected $fillable = ['user_id','category_id','product_type','sku', 'subcategory_id', 'childcategory_id', 'attributes', 'name', 'photo', 'size','size_qty','size_price', 'color', 'details','price','previous_price','stock','policy','status', 'views','tags','featured','best','top','hot','latest','big','trending','sale','features','colors','ship','meta_tag','meta_description','type','file','link','platform','region','measure','discount_date','is_discount','catalog_id','slug'];
    ...
    public function emptyStock() {
        ...
        $size_stock = 0;
        if(!empty($this->size_qty)){
            $size_stock = array_sum($this->size_qty);
        }
        if(($stck == "0" || $stck == '') & $size_stock < 1){
            return true;            
        }
    }
