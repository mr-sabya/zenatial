...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $datas = $user->products()->search()->latest()->paginate($per_page);     
        //$datas = $user->products()->where('product_type','normal')->orderBy('id','desc')->get();    
        return view('vendor.product.index', compact('datas', 'per_page'));
    }
    ...
    public function createProduct()
    {
        $cats = Category::all();
        $sign = Currency::where('is_default','=',1)->first();
        return view('vendor.product.create',compact('cats','sign'));
    }    
    ...
    public function store(Request $request)
    {
        ...
        $input['previous_price'] = ($request->previous_price / $sign->value);
